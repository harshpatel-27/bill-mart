...
interface DataState {
  ...
  updateCustomer: (value: Models.Document) => void;
  ...
  updateInvoice: (value: Models.Document) => void;
...
export const useDataStore = create<DataState>()((set, get) => ({
  ...
  updateCategory: (value) =>
    set({
      categories: get()?.categories.map((ele) => {
        if (ele.$id == value.$id) {
          ele = value;
        }
        return ele;
  ...
  updateCustomer: (value) =>
    set({
      customers: get()?.customers.map((ele) => {
        if (ele.$id == value.$id) {
          ele = value;
        }
        return ele;
